...
import { Share2, Crosshair, Map as MapIcon, AlertCircle } from 'lucide-react';
...
const convertDocToAlert = (doc: QueryDocumentSnapshot<DocumentData>): Alert => {
    ...
    return {
        ...
        initiatorUserId: Number(data.initiatorUserId) || 0, // Convert string ID to number
        description: data.description || ''
    };
};

const MapView = React.forwardRef<any, MapViewProps>(({ alerts, center, zoom, onClose }, ref) => {
        ...
        const unsubscribe = onSnapshot(q, 
            (snapshot) => {
                try {
                    ...
                    setError('Error processing alert data: ' + handleFirebaseError(err));
                }
            },
            (error) => {
    ...
    return (
        <div style={{ position: 'relative' }}>
            ...
            >
                        ...
                        >
                            <Popup>
                                <div className="popup-content">
                                    <h3 className={`text-${alert.status === 'Active' ? 'danger' : 'success'}`}>
                                        {alert.type}
                                    </h3>
                                    ...
                                    <p><strong>Initiator:</strong> {alert.initiatorName}</p>
                                </div>
                            </Popup>
                        </Marker>
                ...
                {/* Filter Controls */}
                <div className="leaflet-top leaflet-left" style={{ top: '80px', left: '10px' }}>
                ...
                {/* Map Controls */}
                <div className="leaflet-bottom leaflet-right" style={{ bottom: '120px', right: '10px' }}>
                    <div className="d-flex flex-column align-items-end">
                        <OverlayTrigger
                            placement="left"
                            overlay={<Tooltip id="toggle-view-tooltip">
                        ...
                        <OverlayTrigger
                            placement="left"
                            overlay={<Tooltip id="heatmap-tooltip">
